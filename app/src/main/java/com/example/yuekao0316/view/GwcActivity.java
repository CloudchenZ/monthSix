...
import com.example.yuekao0316.adapter.GouWcAdapter;
...
public class GwcActivity extends AppCompatActivity {
    ...
    private  List<GwcEntity> gwcEntities;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        daoSession = DaoMaster.newDevSession(GwcActivity.this, "akb.db");
        gwcEntities = daoSession.loadAll(GwcEntity.class);
        final GouWcAdapter gouWcAdapter = new GouWcAdapter(R.layout.list_gwc, gwcEntities);
        rvList.setAdapter(gouWcAdapter);
        rvList.setLayoutManager(new LinearLayoutManager(this));
        int size = gwcEntities.size();

        int sumnum = 0;
        for (int i = 0; i < gwcEntities.size(); i++) {
            int num = gwcEntities.get(i).getNum();
            sumnum = sumnum + num;
        }
        btsuan.setText("去结算（"+sumnum+")");
        money.setText("总金额"+sumnum*1000);
        btsuan.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                payV2();
                daoSession.deleteAll(GwcEntity.class);
                finish();
            }
        });
    }
    ...
    private void initView() {
        ...
        cbq = findViewById(R.id.cbq);
        money = findViewById(R.id.money);
        btsuan = findViewById(R.id.btsuan);
    }
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        showAlert(GwcActivity.this, getString(R.string.pay_success) + payResult);
                    } else {
                        // 该笔订单真实的支付结果，需要依赖服务端的异步通知。
                        showAlert(GwcActivity.this, getString(R.string.pay_failed) + payResult);
                    }
                    break;
                }
                case SDK_AUTH_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000") && TextUtils.equals(authResult.getResultCode(), "200")) {
                        ...
                        // 传入，则支付账户为该授权账户
                        showAlert(GwcActivity.this, getString(R.string.auth_success) + authResult);
                    } else {
                        // 其他状态值则为授权失败
                        showAlert(GwcActivity.this, getString(R.string.auth_failed) + authResult);
                    }
                    break;
                }
                default:
    ...
    public void authV2(View v) {
        ...
        Runnable authRunnable = new Runnable() {
            ...
            public void run() {
                // 构造AuthTask 对象
                AuthTask authTask = new AuthTask(GwcActivity.this);
                // 调用授权接口，获取授权结果
